...
path <- '../Grad/Papers/norbornene homopolymerization/cell culture/same mechanics/'
...
df$file2 <- gsub('_001.tif','_001_masks.tif',df$file)
...
df$file_id <- file_map$file_id[match(df$file2,file_map$file_name)]
...
# Plot form factor
###################################################
pdf(paste0(path,'/form_factor_same_mechanics_diff_polyms.pdf'), width=6, height=4)
...
# Plot area
###################################################
pdf(paste0(path,'/projected_area_same_mechanics_diff_polyms.pdf'), width=6, height=4)
...
# Plot aspect ratio
###################################################
pdf(paste0(path,'/aspect_ratio_same_mechanics_diff_polyms.pdf'), width=6, height=5)
...
df$morphology[df$leiden %in% c(7,8,10,18,19,23,25,26)] <- 'Star'
df$morphology[df$leiden %in% c(0,2,12)] <- 'Spindle-1'
df$morphology[df$leiden %in% c(11,13,16,24)] <- 'Spindle-2'
...
###################################################
pdf(paste0(path,'/Fig7_morph_cluster_dynamics.pdf'), width=8, height=2.5)
...
in_dir <- '../Grad/Papers/norbornene homopolymerization/cell culture/same mechanics/cluster_medoids/'
...
#Spindle-2
df$color_code2[df$leiden == 16] = "#D599C0"
df$color_code2[df$leiden == 13] = "#AB3381"
df$color_code2[df$leiden == 11] = "#77004E"
df$color_code2[df$leiden == 24] = "#D599C0"

#Spindle-1
#"#D599C0" "#AB3381" "#77004E" "#3B0027"
df$color_code2[df$leiden == 12] = "#F07543"
df$color_code2[df$leiden == 2] = "#F7BAA1"
df$color_code2[df$leiden == 0] = "#F07543"

#Star
#"#A7D8E9" "#4FB2D4" "#1C7FA1" "#0E3F50"
df$color_code2[df$leiden == 8] = "#A7D8E9"
df$color_code2[df$leiden == 23] = "#1C7FA1" 
df$color_code2[df$leiden == 7] = "#A7D8E9"
df$color_code2[df$leiden == 10] = "#1C7FA1"
df$color_code2[df$leiden == 18] = "#4FB2D4"
df$color_code2[df$leiden == 25] = "#A7D8E9"
df$color_code2[df$leiden == 19] = "#1C7FA1"
df$color_code2[df$leiden == 26] = "#4FB2D4"

#Spherical
#"#A4ADC2" "#4A5B85" "#172852" "#0B1429"
df$color_code2[df$leiden == 21] = "#A4ADC2"
df$color_code2[df$leiden == 17] = "#4A5B85"
df$color_code2[df$leiden == 4] = "#172852"
df$color_code2[df$leiden == 3] = "#A4ADC2"
df$color_code2[df$leiden == 6] = "#0B1429"
df$color_code2[df$leiden == 22] = "#4A5B85"
df$color_code2[df$leiden == 20] = "#172852"
df$color_code2[df$leiden == 14] = "#A4ADC2"
df$color_code2[df$leiden == 1] = "#4A5B85"
df$color_code2[df$leiden == 15] = "#A4ADC2"
df$color_code2[df$leiden == 5] = "#0B1429"
df$color_code2[df$leiden == 9] = "#4A5B85"

###################################################
# Plotting the CAJAL embedding with medoids
###################################################
pdf(paste0(path,'/Fig7_umap_leiden_4color_morph.pdf'), width=8, height=6)
ggplot(df, aes(Dim1, Dim2)) +
  ...
  scale_y_reverse(limits=c(10,-5.5))+
  xlim(-6.2,17.2)+
...
###################################################
pdf(paste0(path,'/Fig7_umap_contour_polymer_v_day_black.pdf'), width=3.5, height=3.5)
...
# Others
###################################################
pdf(paste0(path,'/FigS12_cajal_colored_by_polymer.pdf'), width=5, height=4)
...
pdf(paste0(path,'/FigS12_cajal_contour_polymer_by_day.pdf'), width=6, height=4)
ggplot(df, aes(x=Dim1, y=Dim2, color=polymer))+
